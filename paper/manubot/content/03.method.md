...
- Repositories (ArrayExpress and Gene Expression Omnibus):
  To get the metadata of the dataset that answers the scientific questions, we queried each repository via API, using the same keywords that the researchers had used in their manual query.
  From the found dataset, we retrieved information to calculate the maturity indicators for the principles F2, F3, I1, I3, R1, and R12.
- [re3data.org](www.re3data.org):
  It is a registry containing information about more than 2000 data repositories from various disciplines.
  From querying re3data we computed the maturity indicators for the principles F1, A2, and R12.   
- [Google Dataset Search](https://toolbox.google.com/datasetsearch):  
  It is a search engine specifically for datasets.
  We looked for the titles of datasets to quantify the principle F4. We searched manually as Google Dataset Search does not have an API.

To the majority of metrics we assigned values 1 or 0 for satisfying or not the requirements.
Only the maturity indicator has continue values between 0 and 1, as we calculated it as the ratio between number of keywords found in the metadata and the number of keywords used by the researcher during the manual query.
Identification of metrics was based on string finding. For quick string search, we vectorize metadata expressed in xml


Details about how we calculated the metrics are in Table x and in the Jupyter notebook y.
...
To summarize and compare the outputs of our calculation, we created a visualization where each row corresponds to a user-case and each column to FAIR maturity indicator.
We represent values of FAIR metrics as sizes of the shapes that describe each point. Shapes are square for metric values established manually, circles for metrics established automatically, and crosses for not computed metrics.
Colors represent metric group: blue for findable, red for accessible, green for interoperable, orange for reusable, and grey for non evaluated metrics.
We created the graph using the R library ggplot2 [@doi:10.1007/978-0-387-98141-3]  
