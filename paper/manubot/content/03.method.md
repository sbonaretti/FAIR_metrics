...
We asked researchers in our department for cases where they looked for datasets in a scientific repository to answer a research questions.
We selected two cases that involved different repositories.
...
To the majority of the maturity indicators, we assigned value 1 if the criterion was satisfied and 0 in the opposite case.
To the maturity indicator for F2 we assigned a number between 0 and 1 as we calculated it as the ratio between the number of keywords in the dataset metadata over the total number of keywords used by the researcher in the manual query.
...
In addition, we indicate when we used *(meta)data* as (data) or (metadata), and an automatic (A) or manual (M) procedure for information retrieval.
...
<td>
"doi" icon is enabled in www.re3data.org (data) (M)
</td>
</tr>
...
<td>
Dataset title found in Google Dataset Search (data) (M)
</td>
</tr>
...
<td>
HTTP request returns 200 (data) (A)
</td>
</tr>
...
<td>
*Dataset* metadata contain more information than search keywords (F2) (metadata) (A)
</td>
</tr>
